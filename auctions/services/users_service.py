...
class UsersService:
    ...
    def login_user_from_shop(self, args: dict[str, str]) -> AuthToken:
        token = args.get("token")
        token3 = args.get("token3")
        user_id = args.get("user_id")

        try:
            user = self.users_repository.get_one(User.id == user_id)  # type: ignore
        except ObjectDoesNotExist as exception:
            raise UserDoesNotExist() from exception

        if token is not None:
            if not self.password_service.check_shop_token(token, user):
                raise InvalidPassword()
        elif token3 is not None:
            if not self.password_service.check_shop_token3(token3, args, user):
                raise InvalidPassword()
